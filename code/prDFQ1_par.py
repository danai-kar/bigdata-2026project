...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Order times of day by percentage of crimes committed on the street (parquet files).",
    )
    parser.add_argument("--base-path", help="Base path.")
    parser.add_argument("--crimes1", help="Explicit crimes part1 parquet path.")
    parser.add_argument("--crimes2", help="Explicit crimes part2 parquet path.")
...
def main() -> None:
    args = parse_args()
    crimes1_path = args.crimes1 or "hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/dsml00293/data_parquet/Crimes1.parquet"
     
    crimes2_path = args.crimes2 or "hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/dsml00293/data_parquet/Crimes2.parquet"
    ...
    if output_path is None and args.base_path:
        output_path = build_path(args.base_path, f"DFQ1par_{spark.sparkContext.applicationId}")

    crimes1_df = spark.read.parquet(crimes1_path)
    crimes2_df = spark.read.parquet(crimes2_path)
    ...
    day_part_df = street_crimes.withColumn('period',
