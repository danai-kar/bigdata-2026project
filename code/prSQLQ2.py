...
def main() -> None:
    ...
    crimes1_df.createOrReplaceTempView('crimes1')
    crimes2_df.createOrReplaceTempView('crimes2')

    result_df = spark.sql(
        """
        SELECT year, month, crime_total, ranking
        FROM (
            SELECT year, month, crime_total, ROW_NUMBER() OVER(
                PARTITION BY year
                ORDER BY crime_total DESC
                ) AS ranking
            FROM (
                SELECT YEAR(to_timestamp(`DATE OCC`, 'yyyy MMM dd hh:mm:ss a')) AS year, MONTH(to_timestamp(`DATE OCC`, 'yyyy MMM dd hh:mm:ss a')) AS month, COUNT(*) AS crime_total
                FROM crimes1 
                GROUP BY year, month
                UNION  ALL 
                    SELECT YEAR(to_timestamp(`DATE OCC`, 'yyyy MMM dd hh:mm:ss a')) AS year, MONTH(to_timestamp(`DATE OCC`, 'yyyy MMM dd hh:mm:ss a')) AS month, COUNT(*) AS crime_total
